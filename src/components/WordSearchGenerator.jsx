...
const WordSearchGenerator = () => {
  ...
  // Usamos useRef para evitar dependencias circulares
  const generatePuzzleRef = useRef(null);
  
  // Funciones auxiliares para colocar palabras
  ...
  // Esta función ya no se usa directamente porque usamos generatePuzzle
  ...
  const generatePuzzle = useCallback(() => {
    ...
    const placed = [];
    let workingBoard = JSON.parse(JSON.stringify(emptyBoard));
    ...
    // Si no hemos podido colocar todas las palabras, aumentamos el tamaño del tablero
    ...
    // Forzamos a que todas las palabras estén colocadas
  ...
  }, [rows, cols, horizontalWords, verticalWords, diagonalWords, bottomUpWords, topDownWords]);

  // Asignamos la función al ref para evitar dependencias circulares
  useEffect(() => {
  ...
  const removeWord = useCallback((category, index) => {
    ...
    setTimeout(() => generatePuzzleRef.current && generatePuzzleRef.current(), 0);
  }, []);
  ...
  useEffect(() => {
    // Usamos generatePuzzleRef.current para evitar problemas de dependencias circulares
    if (generatePuzzleRef.current) {
  ...
  useEffect(() => {
    if (board.length > 0 && generatePuzzleRef.current) {
      generatePuzzleRef.current();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rows, cols]);
    ...
    <div className="word-search-container">
      <div className="word-search-panel board-panel">
        ...
        <div className="button-group">
          <button
            onClick={() => generatePuzzleRef.current && generatePuzzleRef.current()}
